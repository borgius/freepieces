...
| `AUTH_STORE_ID` | `00000000000000000000000000000001` |
...
### Why does the script set a placeholder `AUTH_STORE_ID`?

OpenAuth needs a KV namespace for session storage. The placeholder value lets `wrangler dev` start locally. For deployment, create a real KV namespace with `wrangler kv namespace create AUTH_STORE`.
...
`freepieces` exposes two subpath exports for downstream projects that want to deploy their own Cloudflare Worker with custom pieces, without forking this repo.

### Subpath exports

| Import path | Purpose |
| --- | --- |
| `freepieces/worker` | `createFreepiecesWorker()` factory that returns `{ fetch, queue }` |
| `freepieces/framework` | `registerPiece`, `registerApPiece`, `createPiece`, `getPiece`, `listPieces`, types |
...
# or: pnpm add freepieces hono
```

Write your worker entrypoint:
...
Point your `wrangler.toml` at the admin SPA that ships inside the package:
...
You can also use the `fp install` CLI inside your consumer project — it writes the same piece wrapper files and updates your `src/pieces/index.ts` automatically.

A complete example lives in [examples/consumer-worker](../examples/consumer-worker/).
