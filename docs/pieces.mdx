...
- `src/pieces/cloudflare-d1.ts`
- `src/pieces/cloudflare-queue.ts`
- `src/pieces/cloudflare-r2.ts`
- `src/pieces/cloudflare-workflow.ts`
- `src/pieces/gmail.ts`
...
| `src/pieces/cloudflare-d1.ts` | native Cloudflare D1 binding piece |
| `src/pieces/cloudflare-queue.ts` | native Cloudflare Queue binding piece |
| `src/pieces/cloudflare-r2.ts` | native Cloudflare R2 binding piece |
| `src/pieces/cloudflare-workflow.ts` | native Cloudflare Workflow binding piece |
...
## Registering pieces in a consumer project

When you use `freepieces` as a library (see [Install — Use as a library](./install.mdx#use-as-a-library)), the piece registration workflow is the same as inside this repo.

Import `registerPiece` or `registerApPiece` from `freepieces/framework` and call them before (or as a side effect of) importing your worker entrypoint:

```ts
// src/pieces/index.ts (consumer project)
...
```ts
// src/worker.ts (consumer project)
...
The `fp install` CLI also works inside a consumer project that follows the same `src/pieces/index.ts` layout with `// @fp:pieces:start` / `// @fp:pieces:end` markers. Running `fp install @activepieces/piece-airtable` generates the wrapper file and updates the index automatically.

The registry is module-global, so pieces registered in `./pieces/index.js` are visible to `createFreepiecesWorker()` — as long as both resolve the same `freepieces` package instance (keep it as a single dependency, not both a workspace dep and a transitive dep).
