export const meta = {
  slug: 'actions',
  title: 'Actions',
  summary: 'Call `/run/:piece/:action` to execute work immediately and get the result back in the HTTP response.',
  section: 'runtime',
  order: 40,
  estimatedTime: '8 min',
}
...
If you want the full auth picture before you call runtime routes, read [Authentication](./auth.mdx).

Examples:

- send an email with Gmail
- post a Slack message
- query a Cloudflare D1 database binding
- put or fetch an object from a Cloudflare R2 bucket binding
- fetch a record from an API
- call a custom API endpoint
- run a simple API-key-backed operation such as `ping`

If you want event-driven behavior, read [Triggers](./triggers.mdx). If you want time-based polling, read [Polling triggers](./pooling.mdx).
...
Action availability is also per-user. If an action is disabled for the current runtime user, `POST /run/:piece/:action` returns `404 Action not found` for that user.
...
For the full mental model, including admin auth, OAuth storage, token seeding, and webhook verification, read [Authentication](./auth.mdx).
...
- `X-User-Id` selects a stored OAuth2 token from KV and the per-user action visibility state
...
Bundled Cloudflare binding pieces are native no-auth pieces. They operate on bindings already attached to the Worker, so runtime callers only need the normal `RUN_API_KEY` gate when it is configured.

`cloudflare-d1`, `cloudflare-r2`, `cloudflare-queue`, and `cloudflare-workflow` wrap Worker bindings. See [Cloudflare binding pieces](./cloudflare-bindings.mdx) for binding names and examples.
...
- `404` with `{ "error": "Action not found" }` when the name is wrong or the action is disabled for the current runtime user
...
Use `databaseBinding` if your binding is not named `DB`.
...
Use `bucketBinding` if your binding is not named `BUCKET`.
...
- whether the action is enabled for the current runtime user
