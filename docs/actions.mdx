...
`cloudflare-d1` uses `DB` by default and supports `query`, `first`, and `execute`. Pass `databaseBinding` when your Worker binding has another name, and prefer `?` placeholders with `params` instead of interpolating values into SQL.

`cloudflare-r2` uses `BUCKET` by default and supports `put_object`, `get_object`, `delete_object`, and `list_objects`. Pass `bucketBinding` when your Worker binding has another name.
