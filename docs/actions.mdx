...
### Cloudflare binding pieces, such as D1 and R2
...
`cloudflare-d1` uses a D1 binding named `DB` by default and supports:

- `query` — run a prepared statement and return all rows
- `first` — run a prepared statement and return the first row or column value
- `execute` — run an INSERT, UPDATE, DELETE, or DDL statement

Pass `databaseBinding` when your Worker binding is not named `DB`. Prefer `?` placeholders with `params` instead of interpolating values into SQL.

`cloudflare-r2` uses an R2 binding named `BUCKET` by default and supports:
...
Pass `bucketBinding` when your Worker binding is not named `BUCKET`.
